...
import { AlertTriangle, CheckCircle2, Contact2, Search, Sparkles, XCircle } from 'lucide-react';
...
import {
  useEnrichLeadsWithLinkedIn,
  type EnrichLeadsWithLinkedInResult,
 ...
 * (industry, size, description) plus a decision-maker search. Runs
 * synchronously against a bounded batch — each lead is one or two slow Apify
 * actor runs, so selection is capped rather than queued as a background job.
 *
...
export function LinkedInEnrichmentView() {
  ...
  const [result, setResult] = React.useState<EnrichLeadsWithLinkedInResult | null>(null);
  ...
  const leads = useBackendBusinesses({
    ...
    // accident and burn another billed Apify run on a repeat.
  ...
  const enrich = useEnrichLeadsWithLinkedIn();
    ...
    enrich.mutate(
      { businessIds: [...selected], maxDecisionMakersPerCompany: maxDecisionMakers },
      {
        onSuccess: (data) => {
          ...
          if (data.failed > 0) {
            toast.warning(
              `${data.enriched} enriched, ${data.failed} failed, ${data.peopleFound} decision-maker(s) found.`,
            );
          } else {
            toast.success(
              `Enriched ${data.enriched} lead(s), found ${data.peopleFound} decision-maker(s).`,
            );
          }
          setSelected(new Set());
        },
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1.5fr)_minmax(0,1fr)]">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Choose up to {MAX_SELECTABLE} leads to enrich in one run — each is a billed Apify
              event.
            </CardDescription>
          </CardHeader>
        ...
        <Card className="h-fit">
          ...
          <CardContent className="space-y-5">
            ...
            <p className="text-xs text-muted-foreground">
              Found people appear under People, linked to the lead. This can take a while for a
              full batch — the actor runs one lead at a time behind the scenes.
            </p>
          </CardContent>
        </Card>
      </div>
      ...
      {result && (
        <Card className="mt-6">
          <CardHeader>
            ...
            <CardDescription>
              {result.enriched} enriched, {result.failed} failed, {result.peopleFound}{' '}
              decision-maker(s) found across {result.requested} requested.
            </CardDescription>
          </CardHeader>
          <CardContent>
